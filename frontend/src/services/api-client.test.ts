...
describe('ApiClient', () => {
  ...
  it('retains the last valid response and marks it stale when a later request fails', async () => {
    const fetcher = vi.fn<typeof fetch>()
      .mockResolvedValueOnce(jsonResponse({ memoryTotalBytes: 128 * 1024 ** 3, memoryAvailableBytes: 32 * 1024 ** 3, modelMemoryBudget: { source: 'linux-memavailable', totalMiB: 128 * 1024, freeMiB: 32 * 1024, safetyReserveMiB: 128 * 102.4, allocatableMiB: 19660.8 } }))
    ...
    expect(fresh.data.modelMemoryBudget).toMatchObject({ source: 'linux-memavailable', freeGiB: 32, allocatableGiB: 19.2 })
